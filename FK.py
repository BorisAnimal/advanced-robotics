...
def FK_leg(T_base, T_tool, q_active, q_passive, theta, link, platform_link):
...
def FK_tripteron(T_base, T_tool, q_active, q_passive, theta, link, platform_link):
    T = []
    for leg in range(len(T_base)):
        T_leg = FK_leg(T_base[leg], T_tool[leg], q_active[leg], q_passive[leg], theta[leg], link, platform_link)
        T.append(T_leg)
    return T
